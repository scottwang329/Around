...
const (
	...
	BUCKET_NAME = "around-laioffer-bucket"
)
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Failed to save to ES", http.StatusInternalServerError)
		return
	}

	fmt.Println("Post is save to elasticsearch")
}
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	// Seach posts from elasticsearch
	query := elastic.NewGeoDistanceQuery("location")
	query = query.Lat(lat).Lon(lon).Distance(ran)
	...
	if err != nil {
		http.Error(w, "Failed to parse post into JSON object", http.StatusInternalServerError)
		fmt.Printf("Failed to parse json into JSON format %v.\n", err)
		return
	}
	w.Write(js)
}
